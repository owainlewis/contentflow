...
export function ExternalField({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  ...
  return <label className="resource-field"><span>{label}</span><div className="resource-input"><input type="url" aria-label={label} value={value} placeholder="https://…" onChange={(event) => onChange(event.target.value)} aria-invalid={!!value && !valid} />{valid && <a href={value} target="_blank" rel="noreferrer" aria-label={`Open ${label}`}><ExternalLink size={16} /></a>}</div>{value && !valid && <small role="status">Enter a complete http or https link to save.</small>}</label>;
}

export default function PieceEditor({ detail, topics, onChange, showMetadata = true }: { detail: ContentDetail; topics: ContentSummary[]; onChange: (detail: ContentDetail) => void; showMetadata?: boolean }) {
  ...
  if (detail.type === "topic" && "source" in content) return <div className="resource-editor"><ExternalField label="Source document" value={content.source_url} onChange={(source_url) => patch({ content: { ...content, source_url } })} />{textField("Topic notes", "source", content.source, 3)}</div>;
  ...
  return <div className="resource-editor">
    ...
    <div className="piece-metadata">
      ...
      {showMetadata && <label className="resource-field"><span>Status</span><select aria-label="Content status" value={detail.status} onChange={(event) => patch({ status: event.target.value as ContentDetail["status"] })}>{contentStatuses.map((status) => <option key={status} value={status}>{statusLabels[status]}</option>)}</select></label>}
    ...
    <label className="resource-field"><span>Topic group</span><select aria-label="Topic group" value={detail.topic_id ?? ""} onChange={(event) => patch({ topic_id: event.target.value })}><option value="">Standalone piece</option>{topics.map((topic) => <option key={topic.id} value={topic.id}>{topic.working_title || "Untitled topic"}</option>)}</select></label>
    <ExternalField label={youtube ? "YouTube script document" : "External document"} value={detail.document_url ?? ""} onChange={(document_url) => patch({ document_url })} />
    <ExternalField label="Frame.io / media link" value={detail.video_url ?? ""} onChange={(video_url) => patch({ video_url })} />
    {youtube ? <>
      {textField("YouTube title", "publishing_title", youtube.publishing_title, 1)}
      {textField("YouTube description", "description", youtube.description)}
      {[youtube.topic, youtube.icp, youtube.angle, youtube.cta].some(Boolean) && <details><summary>Stored video brief</summary>{textField("YouTube topic", "topic", youtube.topic, 2)}{textField("YouTube ICP", "icp", youtube.icp, 2)}{textField("YouTube angle", "angle", youtube.angle, 2)}{textField("YouTube CTA", "cta", youtube.cta, 2)}</details>}
      {youtube.sections.length > 0 && <details className="legacy-script"><summary>Stored script sections</summary>{youtube.sections.map((section, index) => <label className="resource-field" key={section.clientKey}><span>{section.title}</span><AutoTextarea aria-label={`${section.title} script`} minRows={3} value={section.body} onChange={(event) => patch({ content: { ...youtube, sections: youtube.sections.map((item, position) => position === index ? { ...item, body: event.target.value } : item) } })} /></label>)}</details>}
      <details><summary>Recording transcript</summary>{textField("YouTube transcript: what was actually said", "transcript", youtube.transcript)}</details>
    </> : <>
