...
export default function Home() {
  ...
  return <main className="app-shell app-redesign">
    ...
    {view === "workspace" && <section className="editor-panel focus-workspace" aria-label="Content editor">
      ...
      {selected ? <>
        <div className="editor-toolbar"><div className="editor-context"><Button className="text-button toolbar-back" onClick={goBack}><ArrowLeft size={16} />{openedFrom === "weekly" ? "Back to week" : selected?.topic_id ? "Back to topic" : selected?.type === "topic" ? "Back to topics" : "Back to library"}</Button><span className="type-pill" style={{ color: typeMeta[selected.type].color }}><TypeIcon type={selected.type} />{typeMeta[selected.type].label}</span><label className="status-select"><span className={`status-dot ${selected.status}`} /><Select aria-label="Content status" value={selected.status} disabled={editorLocked} onChange={(event) => updateSelected((current) => ({ ...current, status: event.target.value as ContentStatus }))}>{contentStatuses.map((status) => <option value={status} key={status}>{statusLabels[status]}</option>)}</Select><ChevronDown size={14} /></label>{selected.type !== "topic" && <label className={`schedule-chip ${selected.scheduled_at ? "" : "unscheduled"}`}><CalendarDays size={14} /><span>{selected.scheduled_at ? shortDate.format(new Date(selected.scheduled_at)) : "Add date"}</span><Input className="schedule-chip-input" type="date" aria-label="Scheduled date" disabled={editorLocked} onClick={(event) => { try { event.currentTarget.showPicker(); } catch { /* The native control still opens on its own. */ } }} value={selected.scheduled_at ? dayKey(new Date(selected.scheduled_at)) : ""} onChange={(event) => updateSelected((current) => ({ ...current, scheduled_at: event.target.value ? scheduledAtFor(event.target.value) : undefined }))} /></label>}</div><div className="editor-actions">{selectedSchedulePending ? <span className="saved-state saving" role="status"><LoaderCircle className="spin" size={14} />Updating schedule…</span> : <span className={`saved-state ${currentSaveState}`} aria-live="polite">{currentSaveState === "saving" || currentSaveState === "retrying" ? <LoaderCircle className="spin" size={14} /> : currentSaveState === "conflict" || currentSaveState === "error" ? <AlertTriangle size={14} /> : <Check size={14} />}{saveLabel(currentSaveState)}</span>}<Button className="delete-button" disabled={lifecycleDisabled} onClick={() => setDeleteOpen(true)}><Trash2 size={15} /> Delete</Button></div></div>
    ...
    {createOpen && <div className="modal-backdrop"><section ref={createModalRef} className="modal-card" role="dialog" aria-modal="true" aria-labelledby="create-title"><div className="modal-header"><div><h2 id="create-title">What are you creating?</h2><p>Choose a format. You can change its status as the work develops.</p></div><Button className="icon-button" onClick={() => setCreateOpen(false)} aria-label="Close create dialog"><X size={19} /></Button></div><label className="resource-field"><span>Topic group</span><Select aria-label="Create in topic group" value={createTopicId} onChange={(event) => setCreateTopicId(event.target.value)}><option value="">Standalone piece</option>{allSummaries.filter((item) => item.type === "topic").map((topic) => <option key={topic.id} value={topic.id}>{displayTitle(topic)}</option>)}</Select></label><Button className="secondary-button" disabled={createPending} onClick={() => void createItem("topic")}>New topic group</Button><div className="create-grid" aria-busy={createPending}>{enabledTypes.map((type) => <Button key={type} disabled={createPending} onClick={() => void createItem(type, undefined, createTopicId)}><span className="create-icon" style={{ color: typeMeta[type].color }}><TypeIcon type={type} size={19} /></span><span><strong>{typeMeta[type].label}</strong><small>{typeMeta[type].description}</small></span><ChevronRight className="create-arrow" size={16} aria-hidden="true" /></Button>)}</div>{createPending && <p aria-live="polite">Creating…</p>}{actionError && <p className="inline-error" role="alert">{actionError}</p>}</section></div>}
    {deleteOpen && selected && <div className="modal-backdrop"><section ref={deleteModalRef} className="modal-card confirm-card" role="alertdialog" aria-modal="true" aria-labelledby="delete-title" aria-describedby="delete-description"><div className="modal-header"><div><h2 id="delete-title">Delete “{displayTitle(selected)}”?</h2><p id="delete-description">This removes the item immediately and cannot be undone.</p></div></div><div className="confirm-actions"><Button onClick={() => setDeleteOpen(false)}>Cancel</Button><Button className="danger-button" onClick={() => void confirmDelete()}><Trash2 size={15} /> Delete permanently</Button></div></section></div>}
  </main>;
}
