...
Content and topic groups are retained until explicitly deleted. Authentication records and mutation receipts still expire and are cleaned up separately. The API retains `expires_at` for compatibility, with a fixed year-9999 value for content. Existing content is migrated to permanent retention; already deleted content cannot be restored.
